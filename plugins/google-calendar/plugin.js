...
class Plugin extends AppPlugin {
    ...
    registerCollectionTools() {
        ...
        window.syncHub.registerCollectionTools({
            collection: 'Calendar',
            description: 'Calendar events from Google Calendar and other sources',
            schema: {
                title: 'Event title',
                time_period: 'Start time (DateTime)',
                calendar: 'Primary | Work | Personal | Family',
                status: 'Confirmed | Tentative | Cancelled',
                location: 'Event location',
                attendees: 'List of attendees',
                meet_link: 'Video call link',
            ...
            tools: [
                {
                    name: 'find',
                    description: 'Find calendar events by date range, calendar, or status. Returns GUIDs - use [[GUID]] in your response to create clickable links.',
                    parameters: {
                        ...
                        upcoming: { type: 'boolean', description: 'Only future events', optional: true },
                        limit: { type: 'number', optional: true }
                    },
                    handler: async (args, data) => this.toolFindEvents(args, data)
                },
                {
                    name: 'today',
                    description: 'Get today\'s events. Returns GUIDs - use [[GUID]] to link.',
                    parameters: {},
                    handler: async (args, data) => this.toolTodayEvents(args, data)
                },
                {
                    name: 'upcoming',
                    description: 'Get upcoming events for the next N days. Returns GUIDs - use [[GUID]] to link.',
                    parameters: {
                        days: { type: 'number', description: 'Number of days to look ahead (default 7)', optional: true },
                        limit: { type: 'number', optional: true }
                    },
                    handler: async (args, data) => this.toolUpcomingEvents(args, data)
                },
                {
                    name: 'needs_followup',
    ...
    async toolFindEvents(args, data) {
        const collections = await data.getAllCollections();
        const calendar = collections.find(c => c.getName() === 'Calendar');
        if (!calendar) return { error: 'Calendar collection not found' };

        const records = await calendar.getAllRecords();
        let results = records;
    ...
    async toolTodayEvents(args, data) {
        const collections = await data.getAllCollections();
        const calendar = collections.find(c => c.getName() === 'Calendar');
        if (!calendar) return { error: 'Calendar collection not found' };
        ...
        return results.map(r => ({
            ...
            calendar: r.prop('calendar')?.choice(),
            location: r.text('location'),
            prep_done: r.prop('prep')?.checkbox()
        }));
    }

    async toolUpcomingEvents(args, data) {
        const collections = await data.getAllCollections();
        const calendar = collections.find(c => c.getName() === 'Calendar');
        if (!calendar) return { error: 'Calendar collection not found' };
    ...
    async toolNeedsFollowup(args, data) {
        ...
        let results = records.filter(r => r.prop('followup')?.checkbox() === true);
